"""
Module for MCMC sampling of a flat ΛCDM model using supernova distance modulus data
to obtain nuisance parameters.

This script handles:
- Loading fitted light curve parameters.
- Estimating the distance modulus and filtering data.
- Defining the likelihood and prior functions.
- Performing MCMC sampling with emcee.
- Plotting and summarizing the posterior distributions.

Author: Alejandro Cartes
"""

from utils import *
from d_distance_modulus import *
from multiprocessing import Pool

def load_and_filter_data(filename="lc_params.pkl"):
    """
    Load fitted light curve data and filter
    """

    data = load_fitted_data(name_file=filename)
    return filter_data(data)

# MCMC objective function
def chi2_marg(params, data, sigma_int=0, reduced=False):
    """
    Compute the marginalized chi² for a flat ΛCDM model
    """

    alpha, beta, gamma = params

    omega_m = 0.3
    omega_de = 1 - omega_m

    mu_marg_th = distance_modulus(data.z, omega_m=omega_m, omega_de=omega_de)

    data_fitted = estimate_distance_modulus(data, alpha=alpha, beta=beta, gamma=gamma,
                                            sigma_int=sigma_int)
    
    delta_mu_marg = data_fitted.mu - mu_marg_th
    
    A = np.sum(delta_mu_marg ** 2 / data_fitted.mu_err ** 2)
    B = np.sum(delta_mu_marg / data_fitted.mu_err ** 2)
    C = np.sum(1 / data_fitted.mu_err ** 2)
    
    chi_squared = A + np.log(C / (2 * np.pi)) - B ** 2 / C
    dof = np.shape(data)[0] - (len(params) + 1)

    return chi_squared / dof if reduced else chi_squared

prior_lims = [(0.0, 0.5),  # alpha
              (1.0, 5.0),  # beta
              (-0.5, 0.5)]  # gamma

def log_prior(params):
    """
    Define the prior
    """
    conds = [prior_lims[i][0] < param < prior_lims[i][1]
             for i, param in enumerate(params)]
    
    if all(conds):
        return 0.0
    
    return -np.inf

def log_likelihood(params, data):
    """
    Compute the log-likelihood function
    """
    chi_squared = chi2_marg(params, data)
    log_norm = 0  # np.sum(np.log(2 * np.pi * data.mu_err ** 2))
    return -0.5 * (chi_squared + log_norm)

def log_posterior(params, data):
    """
    Compute the log-posterior (log-prior + log-likelihood)
    """
    lp = log_prior(params)
    
    if not np.isfinite(lp):
        return -np.inf
    
    return lp + log_likelihood(params, data)

def best_params_max_posterior(data, x0):
    """
    Find the best-fit by maximizing the posterior
    """
    best_params = optimize.minimize(lambda params: -log_posterior(params, data),
                                    x0=x0, method="Powell")
    print(f"Best-fit: {best_params}")
    return best_params.x

def run_mcmc(data, best_params, nwalkers=50, nsteps=5000, load=False,
             filename="flat_lcdm_nuisance.h5"):
    """
    Perform MCMC sampling with emcee
    """
    # Set up backend
    filename = f"data_folder/emcee/{filename}"
    backend = emcee.backends.HDFBackend(filename)
    
    if load:
        sampler = backend

    else:
        # alpha, beta, gamma = best_params
        # param_initial = {'alpha': alpha + np.random.uniform(-1e-1, 1e-1, nwalkers),
        #                 'beta': beta + np.random.uniform(-1e-1, 1e-1, nwalkers),
        #                 'gamma': gamma + np.random.uniform(-gamma, 1e-2, nwalkers),
        #                 }

        # ndim = len(param_initial)
        # p0 = np.stack(list(param_initial.values()), axis=1)

        ndim = len(prior_lims)
        p0 = np.array([np.random.uniform(low, high, nwalkers)
                       for low, high in prior_lims]).T
        
        backend.reset(nwalkers, ndim)

        # Run MCMC
        with Pool() as pool:
            sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior, args=(data,),
                                            backend=backend, pool=pool)
            sampler.run_mcmc(p0, nsteps, progress=True)

    return sampler

def load_nuisance_params(filename="flat_lcdm_nuisance.h5"):
    """
    
    """
    filename = f"data_folder/emcee/{filename}"
    sampler = emcee.backends.HDFBackend(filename)
    labels = [r'$\alpha$', r'$\beta$', r'$\gamma$']
    params_dict = plot_corner(sampler, labels)

    return params_dict

if __name__ == "__main__":
    # Step 1: Load and filter data
    data = load_and_filter_data(filename='classified_Ia_0.9.pkl')
    data = data[data.z < 0.1]  # low redshift data

    # Step 2: Minimize chi²
    x0 = [0.14, 3.20, 0.0]
    best_params = best_params_max_posterior(data, x0=x0)

    # Step 3: Run MCMC sampling
    nwalkers, nsteps = 5_000, 450
    sampler = run_mcmc(data, best_params,
                       nwalkers=nwalkers, nsteps=nsteps,
                       load=True,
                       filename='nuisance_low_z.h5')

    # Step 4: Plot results
    labels = [r'$\alpha$', r'$\beta$', r'$\gamma$']
    plot_chains(sampler, labels=labels)

    params_dict = plot_corner(sampler, labels, truths=x0, pretty=True)
