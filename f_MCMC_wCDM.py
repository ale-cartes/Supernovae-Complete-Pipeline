"""
Module for MCMC sampling of a wCDM model using supernova distance modulus data.
...
def load_and_preprocess_data(file_fitted_data="lc_params.pkl",
    ...
    # alpha_err, beta_err, gamma_err = 0.001, 0.03, 0.007
...
def run_mcmc(data, best_params, nwalkers=50, nsteps=5000, load=False,
    ...
    if load:
        ...
        # omega_m, omega_de, w = best_params
        ...
        ndim = len(prior_lims)
        p0 = np.array([np.random.uniform(low + 1e-6, high, nwalkers)
...
if __name__ == "__main__":
    ...
    sampler = run_mcmc(data, best_params,
                       ...
                       load=True,
                       filename="test_wcdm_2.h5")

    # Step 4: Plot results
    labels = [r'$\Omega_m$', r'$\Omega_{de}$', r'$w$']
